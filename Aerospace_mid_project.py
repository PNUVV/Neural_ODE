...
def train_ode_model(hidden_dim,num_layers, learning_rate, epochs, device):
    ...
    for epoch in range(epochs):
        ...
        for batch_start in range(0, train_size, batch_size):
            batch_x_train = x_train[batch_start:batch_start+batch_size].to(device)
            batch_t_train = t_train[batch_start:batch_start+batch_size].to(device)
...
def train_ode_models(n_samples, hidden_dim,num_layers, learning_rate, epochs, save_path, device):
    ...
    for idx in range(model_try):
        ...
        func, losses, val_losses = train_ode_model(hidden_dim,num_layers, learning_rate, epochs, device) # Function to be defined
...
for n_samples, hidden_dim, learning_rate, epochs in zip(n_samples_list, hidden_dim_list, learning_rate_list, epochs_list):
    ...
    # 최적의 모델 훈련
    best_func, max_r_squared_model,x_pred_train_best,x_pred_val_best,x_pred_test_best = train_ode_models(n_samples, hidden_dim,num_layers, learning_rate, epochs, save_path, device)
